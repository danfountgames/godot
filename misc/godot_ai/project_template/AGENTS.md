...
- **No export *tool*.** There is no `Godot_Export…`: do not go looking for one. But
  exporting itself is **not** out of reach — see *Build and release validation* below.
  It is done from the shell rather than through this interface.
...
This is done from the shell, not through the interface — there is no export tool, and
running an exported binary would be shell execution, which nothing here does. That does
**not** make it unavailable.

**Vanilla export templates work with this fork.** The AI tooling is an editor-only
module (`can_build` returns `env.editor_build`), so it is absent from export templates
by construction — the official templates for this engine's version are functionally
identical to ones built from this tree, and there is no need to build any. Install them
under the folder named by `VERSION_FULL_CONFIG`, which is `4.3.dev` here — **not** the
longer string `--version` prints:

```sh
~/.local/share/godot/export_templates/4.3.dev/web_release.zip
```

A web export can then be served locally and driven in a real browser, which satisfies
"run the release build, not only the editor" properly. Only report release verification
as unavailable once you have actually tried this.
