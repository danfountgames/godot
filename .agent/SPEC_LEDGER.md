...
| X4 | Semantic Inspector documentation capture for Resources and scene nodes | VERIFIED | `tools/mcp_capture_tools.cpp`, deferred cancellation cleanup in `mcp_deferred.cpp` | `tests/test_mcp_tools.h`, `tests/test_mcp_deferred.h`, `run_editor_e2e.py` (native macOS + headless) | native macOS E2E expands `GradientTexture2D.gradient`, highlights `offsets`, unfolds explicit contextual properties, and saves a tight Inspector-only PNG; scene-node mode opens another tab, selects `Section/Target`, and captures `text`. Raincaster production verification created and visually inspected focused IN-02/IN-04/ST-01/ST-03 crops, including multiple named Inspector groups with meaningful preview content. Calls restore the original scene, Inspector, folds, scroll and dock state; headless calls refuse explicitly | none |
| X5 | Semantic Scene tree documentation capture | VERIFIED | `tools/mcp_capture_tools.cpp` | `tests/test_mcp_tools.h`, `run_editor_e2e.py` (native macOS + headless) | opens another tab's named scene, traverses and unfolds the ancestor chain for `Section/Target`, selects/highlights the exact Tree row, centers it with independent context above/below, saves and returns a Scene-dock-only PNG, then demonstrably restores the original scene, selection, folds, scroll and dock tab. Raincaster ST-01 and ST-03 outputs were visually checked to contain the selected node, required parents/siblings and no editor/desktop background | none |
