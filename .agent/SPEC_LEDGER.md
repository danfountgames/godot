...
| ID | Spec | Requirement | Deps | Status | Code | Tests | Evidence | Remaining |
|---|---|---|---|---|---|---|---|---|
| R1 | MCP/PKG | Stdio transport without engine prints on the stream | — | VERIFIED (reshaped by DEC-0015) | `modules/godot_ai/gateway/*` compiled into the editor; platform mains enter it before engine init | `tools/relay/tests/run_tests.py` (44 cases) against `godot --godot-ai-stdio` | the property that mattered - a protocol-clean stdout - came from process separation and now comes from ordering: the gateway runs before anything can print. No separate binary exists | gateway sources must remain engine-free C++17 |
...
| R8 | CI/PKG | macOS and Windows gateway behaviour | R1 | IMPLEMENTED | `modules/godot_ai/gateway/platform*.{h,cpp}`, hook in all three platform mains | POSIX path: 44/44 on native arm64 macOS and in Linux CI | Windows compiles wherever the Windows editor compiles - the gateway is part of that build now, not a separate cross-compile | runtime verification on a Windows host |
...
| X3 | macOS editor is branded and packaged as GodotAI | VERIFIED | `platform/macos/SCsub`, `platform/macos/platform_macos_builders.py`, `misc/dist/macos/editor_info_plist.template`, `misc/dist/macos_tools.app`, `misc/scripts/make_godot_ai_macos_icon.*` | native arm64 bundle build; bundled `*[godot_ai]*` suite | `bin/GodotAI.app` has `GodotAI` as bundle/display/executable name, embeds the reproducible AI-badged ICNS at every macOS icon size, verifies with `codesign`, launches as a commit-stamped `4.8.dev.custom_build`, and passes 71 cases / 452 assertions | none |
...
| X6 | Port the complete AI editor fork from Godot 4.3 to current upstream Godot 4.8 development | VERIFIED | upstream merge `457470a8d0109b994bb8d6951b3abe5299fb6ee6`; 4.8 API adaptations throughout `modules/godot_ai/`, editor accessors and macOS bundle builder | arm64 editor build; 71/71 module cases (452 assertions); 64/64 relay; headless and native `run_editor_e2e.py`; `tools/tests/run_tests.py` | `bin/godot.macos.editor.dev.arm64` and `bin/GodotAI.app` build successfully; the headless E2E covers refusal paths and every nonvisual tool; native E2E covers real nested-resource/node/tree crops, viewport/window/game pixels, runtime inspection/input/audio/tests, imports, checkpoints and permissions. The Linux-only `run_editor_ui_e2e.py` reports its documented `xdotool` skip on macOS | native Windows runtime verification remains tracked under R8 |
