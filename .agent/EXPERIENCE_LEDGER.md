...
| W8 | Embedded windows draw over the tile chrome | PARTLY DIAGNOSED | `mcp_workspace.cpp` sets the embedders to fit their frame | `.agent/evidence/spike_three_variants.py` | **Found by W6.** Partly a misreading: most of the overdraw in the first screenshot was the *user's own* game, embedded full-size in the Game workspace and still drawing while the GodotAI screen was selected — a separate problem worth its own row. The underlying constraint is real though: An embedded game is a native child window, so it renders above the editor's own controls: a neighbouring game covers the header, status line and buttons of the tiles around it. `GameView` never hits this because it has exactly one embedder filling the screen. The workspace needs chrome laid out clear of every embedded rect - a header strip above the grid rather than inside each tile, or insets the embedders never occupy |
