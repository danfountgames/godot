...
private:
	...
	void _handle_write_chunked_begin(int p_peer_id, const Dictionary &p_msg);
	void _handle_write_chunked_data(int p_peer_id, const Dictionary &p_msg);
	void _handle_write_chunked_end(int p_peer_id, const Dictionary &p_msg);
	...
	// Chunked transfer state (one active transfer per peer).
	struct ChunkedTransfer {
		String rel_path;
		int64_t total_size = 0;
		Vector<uint8_t> data;
	};
	HashMap<int, ChunkedTransfer> chunked_transfers;
