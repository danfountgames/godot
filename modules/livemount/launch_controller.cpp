...
Error LaunchController::launch_project(const String &p_path, const String &p_target_scene) {
	...
	// Step 9b: Apply viewport/stretch settings from the mounted project to the root Window.
	// These are only read during Main::setup(), so we must apply them at runtime.
	{
		SceneTree *st = SceneTree::get_singleton();
		Window *w = st ? st->get_root() : nullptr;
		if (w) {
			int vp_w = ProjectSettings::get_singleton()->get_setting("display/window/size/viewport_width", 0);
			int vp_h = ProjectSettings::get_singleton()->get_setting("display/window/size/viewport_height", 0);
			if (vp_w > 0 && vp_h > 0) {
				w->set_content_scale_size(Size2i(vp_w, vp_h));
				print_line("[LaunchController] Viewport size set to: " + itos(vp_w) + "x" + itos(vp_h));
			}

			String stretch_mode_str = ProjectSettings::get_singleton()->get_setting("display/window/stretch/mode", "disabled");
			if (stretch_mode_str == "canvas_items") {
				w->set_content_scale_mode(Window::CONTENT_SCALE_MODE_CANVAS_ITEMS);
			} else if (stretch_mode_str == "viewport") {
				w->set_content_scale_mode(Window::CONTENT_SCALE_MODE_VIEWPORT);
			} else {
				w->set_content_scale_mode(Window::CONTENT_SCALE_MODE_DISABLED);
			}

			String stretch_aspect_str = ProjectSettings::get_singleton()->get_setting("display/window/stretch/aspect", "ignore");
			if (stretch_aspect_str == "keep") {
				w->set_content_scale_aspect(Window::CONTENT_SCALE_ASPECT_KEEP);
			} else if (stretch_aspect_str == "keep_width") {
				w->set_content_scale_aspect(Window::CONTENT_SCALE_ASPECT_KEEP_WIDTH);
			} else if (stretch_aspect_str == "keep_height") {
				w->set_content_scale_aspect(Window::CONTENT_SCALE_ASPECT_KEEP_HEIGHT);
			} else if (stretch_aspect_str == "expand") {
				w->set_content_scale_aspect(Window::CONTENT_SCALE_ASPECT_EXPAND);
			} else {
				w->set_content_scale_aspect(Window::CONTENT_SCALE_ASPECT_IGNORE);
			}

			float stretch_scale = ProjectSettings::get_singleton()->get_setting("display/window/stretch/scale", 1.0);
			w->set_content_scale_factor(stretch_scale);

			print_line("[LaunchController] Stretch mode: " + stretch_mode_str + ", aspect: " + stretch_aspect_str + ", scale: " + rtos(stretch_scale));
		}
	}

	// Step 9c: Load project translations into TranslationServer.
	// Normally done in Main::setup(), but LiveMount loads projects at runtime.
	{
		TranslationServer *ts = TranslationServer::get_singleton();
		if (ts) {
			ts->load_project_translations(ts->get_main_domain());
			ResourceLoader::load_translation_remaps();
			print_line("[LaunchController] Project translations loaded.");
		}
	}
	...
	// Disable editor_hint so game scripts run normally (e.g. _process enabled).
	// Without this, scripts that check Engine.is_editor_hint() will think
	// they're in the Godot editor and disable animations/gameplay.
...
void LaunchController::stop_project() {
	...
	// Restore editor_hint so LiveMount shell subsystems work correctly.
	...
	// Step 2b: Clear project translations.
	{
		TranslationServer *ts = TranslationServer::get_singleton();
		if (ts) {
			ts->get_main_domain()->clear();
			print_line("[LaunchController] Project translations cleared.");
		}
	}
